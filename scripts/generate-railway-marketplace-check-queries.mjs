#!/usr/bin/env node
/**
 * Regenerates scripts/data/railway-marketplace-check-queries.json from the list below.
 * Edit QUERIES_TEXT when the canonical comparison list changes.
 */
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";

const __dirname = path.dirname(fileURLToPath(import.meta.url));

const QUERIES_TEXT = `
PostgreSQL
MySQL
MariaDB
Redis
KeyDB
Dragonfly
MongoDB
ClickHouse
Activepieces
Actualbudget
Affine
Alexandrie
Anythingllm
Appflowy
Apprise Api
Appsmith
Appwrite
Argilla
Audiobookshelf
Authentik
Autobase
Babybuddy
Bento Pdf
Beszel
Beszel Agent
Bitcoin Core
Bluesky Pds
Booklore
Bookstack
Browserless
Budge
Budibase
Bugsink
Calcom
Calibre Web
Calibre Web Automated Book Downloader
Cap
Castopod
Changedetection
Chaskiq
Chatwoot
Checkmate
Chibisafe
Chroma
Classicpress With Mariadb
Classicpress With Mysql
Classicpress Without Database
Cloudbeaver
Cloudflared
Cloudreve
Cockpit
Code Server
Codimd
Convertx
Convex
Cryptgeon
Cyberchef
Dashy
Databasus
Deno K V
Directus
Directus With Postgresql
Diun
Docker Registry
Docmost
Documenso
Docuseal
Docuseal With Postgres
Dokuwiki
Dolibarr
Dozzle
Dozzle With Auth
Drizzle Gateway
Drupal With Postgresql
Duplicati
Easyappointments
Elasticsearch
Elasticsearch With Kibana
Emby
Embystat
Ente Photos
Ente Photos With S3
Esphome
Evolution Api
Excalidraw
Faraday
Fider
Filebrowser
Fileflows
Firefly
Firefox
Fizzy
Flipt
Flowise
Flowise With Databases
Forgejo
Forgejo With Mariadb
Forgejo With Mysql
Forgejo With Postgresql
Formbricks
Foundryvtt
Freescout
Freshrss
Freshrss With Mariadb
Freshrss With Mysql
Freshrss With Postgresql
Garage
Getoutline
Ghost
Gitea
Gitea With Mariadb
Gitea With Mysql
Gitea With Postgresql
Github Runner
Gitlab
Glance
Glances
Glitchtip
Glpi
Goatcounter
Gotenberg
Gotify
Gowa
Grafana
Grafana With Postgresql
Gramps Web
Grist
Grocy
Hatchet
Heimdall
Heyform
Homarr
Home Assistant
Homebox
Homepage
Hoppscotch
Imgcompress
Immich
Infisical
Invoice Ninja
It Tools
Jellyfin
Jenkins
Joomla With Mariadb
Joplin
Jupyter Notebook Python
Karakeep
Keycloak
Keycloak With Postgres
Kimai
Kuzzle
Labelstudio
Langflow
Langfuse
Leantime
Librechat
Libreoffice
Librespeed
Libretranslate
Limesurvey
Linkding
Linkding Plus
Listmonk
Litellm
Litequeen
Lobe Chat
Logto
Lowcoder
Mage Ai
Mailpit
Marimo
Martin
Matrix Synapse With Postgresql
Matrix Synapse With Sqlite
Mattermost
Mealie
Mediawiki
Meilisearch
Memos
Metabase
Metamcp
Metube
Mindsdb
Minecraft
Miniflux
Mixpost
Moodle
Mosquitto
N8N
N8N With Postgres And Worker
N8N With Postgresql
Navidrome
Neon Ws Proxy
Netbird Client
Newapi
Newt Pangolin
Next Image Transformation
Nextcloud
Nextcloud With Mariadb
Nextcloud With Mysql
Nextcloud With Postgres
Nexus
Nexus Arm
Nitropage
Nitropage With Postgresql
Nocobase
Nocodb
Nodebb
Ntfy
Observium
Odoo
Ollama With Open Webui
Once Campfire
Onedev
Onetimesecret
Open Archiver
Open Webui
Openclaw
Openpanel
Opnform
Orangehrm
Organizr
Osticket
Overseerr
Owncloud
Pairdrop
Palworld
Paperless
Passbolt
Paymenter
Penpot
Penpot With S3
Pgadmin
Pgbackweb
Phpmyadmin
Pi Hole
Plex
Plunk
Pocket Id
Pocket Id With Postgresql
Pocketbase
Portainer
Postiz
Prefect
Privatebin
Prowlarr
Proxyscotch
Pydio Cells
Qbittorrent
Qdrant
Rabbitmq
Radarr
Rallly
Reactive Resume
Readeck
Redlib
Redmine
Rivet Engine
Rocketchat
Rybbit
Ryot
Satisfactory
Seafile
Searxng
Seaweedfs
Sequin
Sessy
Sftpgo
Shlink
Signoz
Silverbullet
Siyuan
Slash
Snapdrop
Soju
Soketi
Soketi App Manager
Sonarr
Spacebot
Sparkyfitness
Statusnook
Stirling Pdf
Strapi
Supabase
Superset With Postgresql
Supertokens With Mysql
Supertokens With Postgresql
Sure
Swetrix
Syncthing
Tailscale Client
Teable
Terraria Server
Tolgee
Traccar
Trailbase
Transmission
Trigger
Triliumnext
Twenty
Typesense
Umami
Unleash With Postgresql
Unleash Without Database
Unstructured
Uptime Kuma
Uptime Kuma With Mariadb
Uptime Kuma With Mysql
Usesend
Vaultwarden
Vert
Vikunja
Vikunja With Postgresql
Vvveb
Vvveb With Mariadb
Vvveb With Mysql
Wakapi
Weaviate
Web Check
Weblate
Whoogle
Wikijs
Windmill
Wings
Wireguard Easy
Wordpress With Mariadb
Wordpress With Mysql
Wordpress Without Database
Yamtrack
Yamtrack With Postgresql
Zipline
`;

const queries = QUERIES_TEXT.trim()
  .split(/\n+/)
  .map((s) => s.trim())
  .filter(Boolean);

const out = path.join(__dirname, "data", "railway-marketplace-check-queries.json");
fs.mkdirSync(path.dirname(out), { recursive: true });
fs.writeFileSync(out, JSON.stringify(queries, null, 2) + "\n");
console.error(`Wrote ${queries.length} queries → ${out}`);
